...
if __name__=='__main__':
	...
	with torch.no_grad():
		def hsmm_mse(hsmm, z_in, x2_gt):
			...
			return F.mse_loss(x2_gt, x2_gen, reduction='mean').detach().cpu().numpy(), x2_gen
		for i in range(NUM_ACTIONS):
			...
			for n in range(len(nb_states)):
				...
				for j in range(len(x2_gt_train)):
					mse_j, x2_gen = hsmm_mse(hsmm, z_encoded_train[j][:, :2*z_dim], x2_gt_train[j])
					mse.append(mse_j)
				train_mse[n, i] = np.sum(mse)

				mse = []
				for j in range(len(x2_gt_test)):
					mse_j, x2_gen = hsmm_mse(hsmm, z_encoded_test[j], x2_gt_test[j])
					mse.append(mse_j)

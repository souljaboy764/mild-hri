...
def run_iteration_both(iterator, hsmm, model_h, model_r, optimizer):
	...
	for i, x in enumerate(iterator):
		...
		if hsmm != []:
			...
			if model_r.window_size > 1 :
				mu_r = torch.Tensor(hsmm[label].mu[:,zh_dim:zh_dim+zr_dim]).to(device)
				Sigma_r = torch.Tensor(hsmm[label].sigma[:, zh_dim:zh_dim+zr_dim, zh_dim:zh_dim+zr_dim]).to(device)
			else:
...
def run_iteration(iterator, hsmm, model_r, optimizer, model_h):
	iters = 0
	total_recon = []
	total_reg = []
	total_loss = []
	...
	for i, x in enumerate(iterator):
		...
		if hsmm != []:
			if model_r.window_size > 1 :
				mu_r = torch.Tensor(hsmm[label].mu[:,zh_dim:]).to(device)
				Sigma_r = torch.Tensor(hsmm[label].sigma[:, zh_dim:, zh_dim:]).to(device)
			else:
				mu_r = torch.Tensor(hsmm[label].mu[:,2*zh_dim:2*zh_dim+zr_dim]).to(device)
				Sigma_r = torch.Tensor(hsmm[label].sigma[:, 2*zh_dim:2*zh_dim+zr_dim, 2*zh_dim:2*zh_dim+zr_dim]).to(device)
			alpha_hsmm, _, _, _, _ = hsmm[label].compute_messages(marginal=[], sample_size=seq_len)
...
if __name__=='__main__':
	...
	parser.add_argument('--human-ckpt', type=str, default='/home/vignesh/playground/hsmmvae/logs/fullvae_rarm_window_07082032_klmaxclass/models/final.pth', metavar='CKPT',
						help='Path to read training and testing data (default: /home/vignesh/playground/hsmmvae/logs/fullvae_rarm_window_07082032_klmaxclass/models/final.pth).')			
	...
	parser.add_argument('--model', type=str, default='FullCovVAE', metavar='ARCH', choices=['AE', 'VAE', 'WAE', 'FullCovVAE'],
						help='Model to use: AE, VAE, FullCovVAE or WAE (default: FullCovVAE).')
	...
	model_h = getattr(networks, args.model)(**(vae_h_config.__dict__)).to(device)
	ckpt = torch.load(args.human_ckpt)
	model_h.load_state_dict(ckpt['model'])
	model_h.eval()
	
	print("Creating Model and Optimizer")
	config_r = robot_vae_config()
	config_r.window_size = model_h.window_size
	model_r = getattr(networks, args.model)(**(config_r.__dict__)).to(device)
	...
	for epoch in range(global_config.EPOCHS):
		...
		train_recon, train_kl, train_loss, xr_gen, zr_post_samples, x, iters = run_iteration(train_iterator, hsmm, model_r, optimizer, model_h)
		...
		with torch.no_grad():
			print('testing model')
			test_recon, test_kl, test_loss, xr_gen, zr_post_samples, x, iters = run_iteration(test_iterator, hsmm, model_r, optimizer, model_h)

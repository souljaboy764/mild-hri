...
class SequenceDataset(Dataset):
	def __init__(self, datafile, train=True, use_vel=True):
		...
		with np.load(datafile, allow_pickle=True) as data:
			...
			for i in range(len(self.traj_data)):
				...
				num_joints = numdims//6

				traj_1 = self.traj_data[i][:, None, numdims//2-3:numdims//2]
				traj_2 = self.traj_data[i][:, None, numdims-3:]
				traj = downsample_trajs([np.concatenate([traj_1, traj_2], axis=-1)], seq_len//4, device)[0, :, 0, :]
				if use_vel:
					vel = np.diff(traj, axis=0, prepend=traj[0:1, :])
					self.traj_data[i] = np.concatenate([traj[:,:3], vel[:,:3], traj[:,3:], vel[:,3:]], axis=-1)
				else:
					self.traj_data[i] = traj
...
class SequenceWindowDataset(Dataset):
	def __init__(self, datafile, train=True, window_length=5):
		device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
		_dataset = SequenceDataset(datafile, train, use_vel=False)
		traj_data = _dataset.traj_data
		labels = _dataset.labels
		self.actidx = _dataset.actidx
		# with np.load(datafile, allow_pickle=True) as data:
		# 	if train:

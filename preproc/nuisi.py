...
for a in range(len(actions)):
	...
	s2 = starts2[a]
	n_demos = num_demos[a]
	data1 = np.load(action+'_agent1.bag.npz',allow_pickle=True)
	trajs_1 = data1['arr_0'] # Nx10x7 : x, y, z, qx, qy, qz, qw
	T1 = np.eye(4)
	T1[:3,3] = trajs_1[0,1,:3]
	T1[:3,:3] = rotation_normalization(trajs_1[0,:,:3] - trajs_1[0,1,:3])
	...
	len1 = len(times_1)
	data2 = np.load(action+'_agent2.bag.npz',allow_pickle=True)
	trajs_2 = data2['arr_0'] # Nx10x7 : x, y, z, qx, qy, qz, qw
	T2 = np.eye(4)
	T2[:3,3] = trajs_2[0,2,:3]
	T2[:3,:3] = rotation_normalization(trajs_2[0,:,:3] - trajs_2[0,2,:3])

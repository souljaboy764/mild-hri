...
if __name__=='__main__':
	...
	parser.add_argument('--results', type=str, default='./logs/2023/vae_hsmmtorch',#+datetime.datetime.now().strftime("%m%d%H%M"), metavar='RES',
	...
	for i in range(NUM_ACTIONS):
		...
		hsmm[-1].init_zeros()
		hsmm[-1].init_priors = torch.ones(nb_states).to(device) / self.nb_states
